...
void Tracker::addContactPoints(const std::vector<float3> &contact_points,
    const int src_model_idx, const int dst_model_idx, const float weight) {
    for (float3 contact_point : contact_points) {
        std::shared_ptr<Prior> p =
            std::make_shared<ContactPrior>(src_model_idx, dst_model_idx, 0, -1, weight,
            contact_point, 100.f, false, false, true);
        addPrior(p);
    }
}

void Tracker::addThumbContact(const float3 &point, const int src_sdf_idx,
    const int src_model_idx, const int dst_model_idx, const float weight) {
