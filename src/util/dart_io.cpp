...
void readFrameXML(const int parent, HostOnlyModel & model, TiXmlElement * element, std::string & dir, const int version) {
    ...
    for (TiXmlElement * child = element->FirstChildElement(); child != 0; child = child->NextSiblingElement()) {
        ...
        if (strcmp(child->Value(),"frame") == 0) {
            ...
            std::string dh_a("0"), dh_alpha("0"), dh_d("0"), dh_theta("0");
            for (TiXmlElement * grandChild = child->FirstChildElement(); grandChild != 0; grandChild = grandChild->NextSiblingElement()) {
                ...
                } else if (strcmp(grandChild->Value(),"dh_offset") == 0) {
                    dh_a     = grandChild->Attribute("a");
                    dh_alpha = grandChild->Attribute("alpha");
                    dh_d     = grandChild->Attribute("d");
                    dh_theta = grandChild->Attribute("theta");
                }
            }
            int ID = model.addFrame(
                parent,jointType,
                posX,posY,posZ,
                oriX,oriY,oriZ,
                axisX,axisY,axisZ,
                jointMin,jointMax,jointName,
                dh_a, dh_alpha, dh_d, dh_theta);
