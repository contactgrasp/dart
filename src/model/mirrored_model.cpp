...
MirroredModel::MirroredModel(const HostOnlyModel & copy,
    ...
    _jointDHParams = new MirroredVector<float4>(numJoints);
    ...
    for (int j=0; j<numJoints; ++j) {
        ...
        _jointDHParams->hostPtr()[j] = copy.getJointDHParam(j);
    ...
    _jointDHParams->syncHostToDevice();
...
void MirroredModel::setPose(const Pose & pose) {
    ...
    for (int f=1; f<getNumFrames(); ++f) {
        ...
        switch(_jointTypes->hostPtr()[joint]) {
            case RotationalJoint:
                T_pf = T_pf * SE3FromDHGraspit(
                    _jointDHParams->hostPtr()[joint].w + p*_jointAxes->hostPtr()[joint].z,
                    _jointDHParams->hostPtr()[joint].z,
                    _jointDHParams->hostPtr()[joint].x,
                    _jointDHParams->hostPtr()[joint].y + p*_jointAxes->hostPtr()[joint].x,
                    p*_jointAxes->hostPtr()[joint].y);
                ++j;
                break;
            case PrismaticJoint:
                T_pf = T_pf * SE3FromDHGraspit(
                    _jointDHParams->hostPtr()[joint].w,
                    _jointDHParams->hostPtr()[joint].z + p*_jointAxes->hostPtr()[joint].z,
                    _jointDHParams->hostPtr()[joint].x + p*_jointAxes->hostPtr()[joint].x,
                    _jointDHParams->hostPtr()[joint].y,
                    p*_jointAxes->hostPtr()[joint].y);
                ++j;
                break;
        }
