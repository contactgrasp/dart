...
int HostOnlyModel::addFrame(int parent, JointType type,
                            ...
                            std::string dh_a, std::string dh_alpha, std::string dh_d, std::string dh_theta) {

    std::vector<std::string> frameExpressions(15);
    ...
    frameExpressions[11] = dh_a;
    frameExpressions[12] = dh_alpha;
    frameExpressions[13] = dh_d;
    frameExpressions[14] = dh_theta;
    ...
    float4 dh_offsets = make_float4(
        evaluateExpression(dh_a),       // x
        evaluateExpression(dh_alpha),   // y
        evaluateExpression(dh_d),       // z
        evaluateExpression(dh_theta));  // w
    _dh_params.push_back(dh_offsets);
