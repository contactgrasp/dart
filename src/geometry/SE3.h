...
inline PREFIX SE3 SE3FromDHGraspit(const float theta, const float d, const float a, const float alpha, const float y_rot) {
    return SE3FromRotationY(y_rot) * SE3FromRotationZ(theta) * SE3FromTranslation(0, 0, d) *
        SE3FromTranslation(a, 0, 0) * SE3FromRotationX(alpha);
}
...
// convention: https://en.wikipedia.org/wiki/Euler_angles#Rotation_matrix
// Tait-Bryan ZYX
inline PREFIX SE3 SE3FromEuler(float3 phiThetaPsi) {
