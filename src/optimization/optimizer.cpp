...
OptimizerNoObs::OptimizerNoObs() {
    init();
}

void OptimizerNoObs::init() {
    _maxModels = 3;
    _maxDims = 100;
...
void OptimizerNoObs::unpack(Eigen::VectorXf & eJ,
                       Eigen::MatrixXf & JTJ,
                       float & e,
                       float * sys,
...
Optimizer::Optimizer(const DepthSourceBase * depthSource, const int predictionWidth,
    const int predictionHeight) : OptimizerNoObs() {
...
Optimizer::Optimizer(const int depthWidth, const int depthHeight, const float2 focalLength,
                     const int predictionWidth, const int predictionHeight): OptimizerNoObs() {
...
void Optimizer::init(const int depthWidth, const int depthHeight,
    const float2 focalLength, const int predictionWidth, const int predictionHeight) {
...
void OptimizerNoObs::computeSelfIntersectionContribution(Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
                                                    const MirroredModel & model, const Pose & pose,
                                                    const OptimizationOptions & opts,
...
void OptimizerNoObs::computeIntersectionContribution(Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
...
float Optimizer::optimizePoses(std::vector<MirroredModel *> & models,
...
float OptimizerNoObs::optimizePoses(std::vector<MirroredModel *> & models,
                              std::vector<Pose> & poses,
                              const float4 * dObsVertMap,
                              const float4 * dObsNormMap,
                              const int width,
...
//            cudaMemset(_dDebugIntersectionError,0,_maxIntersectionSites*sizeof(float));
        ...
        // process the priors' contribution to the system
        // priors manage their own regularization and LM damping
        for (int p=0; p<priors.size(); ++p) {
            priors[p]->resetDstSdfNum();
            priors[p]->computeContribution(sparseJTJ,fullJTe,modelOffsets,priorOffsets[p],models,poses, opts);
        }

        // add regularization and LM damping to the model parts system
        for (int m=0; m < nModels; m++) {
            int rdims = poses[m].getReducedDimensions();
            int offset = modelOffsets[m];
            for (int i = 0; i < rdims; i++) {
                // ensure JTJ is full rank - regularization
                sparseJTJ.coeffRef(offset+i, offset+i) += opts.regularization[m];
                // LM damping
                sparseJTJ.coeffRef(offset+i, offset+i) +=
                    opts.regularizationScaled[m]*sparseJTJ.coeff(offset+i, offset+i);
            }
        }
    ...
    float error = getError(models, poses, opts, T_mcs, collisionClouds,
        intersectionPotentialMatrices, priors);
    return error;
}

float OptimizerNoObs::getError(std::vector<MirroredModel *> & models,
                              std::vector<Pose> & poses,
                              OptimizationOptions & opts,
                              MirroredVector<SE3> & T_mcs,
                              std::vector<MirroredVector<float4> *> & collisionClouds,
    ...
    for (int m=0; m<nModels; ++m) {
        ...
        if (opts.lambdaIntersection[m + m*nModels] > 0) {
            computeSelfIntersectionContribution(eJ,JTJ,intersectionError,model,pose,opts,
                                                *collisionClouds[m],*intersectionPotentialMatrices[m], nModels,
                                                debugIntersectionOffset);
            // std::cout << "Self-Intersection = " << intersectionError << " ";
        ...
        for (int d=0; d<nModels; ++d) {
            ...
            if (opts.lambdaIntersection[m + d*nModels] > 0) {
                computeIntersectionContribution(eJ,JTJ,intersectionError,model,*models[d],pose,opts,
                                                *collisionClouds[m],nModels,debugIntersectionOffset);
                // std::cout << "Intersection = " << intersectionError << " ";
                error += intersectionError;
            }
        }
    ...
    for (int p=0; p<priors.size(); ++p) {
        priors[p]->resetDstSdfNum();
        priors[p]->computeContribution(sparseJTJ,fullJTe,modelOffsets,priorOffsets[p],models,poses, opts);
        pe += priors[p]->getError();
        // std::cout << priors[p]->getError() << std::endl;
        // if (priors[p]->isInverted()) ie += priors[p]->getError();
        // else e += priors[p]->getError();
    }
    error += pe / priors.size();
    // std::cout << "Contact energy = " << e << ", No-contact energy = " << ie << std::endl;
